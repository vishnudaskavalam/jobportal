...
export class SeedActualData1780427439614 implements MigrationInterface {
    ...
    public async up(queryRunner: QueryRunner): Promise<void> {
        ...
        const engineeringCategory = await queryRunner.query(`SELECT id FROM "categories" WHERE name = 'Engineering' LIMIT 1`);
        const designCategory = await queryRunner.query(`SELECT id FROM "categories" WHERE name = 'Design' LIMIT 1`);
    ...
    public async down(queryRunner: QueryRunner): Promise<void> {
        await queryRunner.query(`DELETE FROM "jobs" WHERE company IN ('TechCorp Inc.', 'Creative Studio', 'StartupX')`);
        await queryRunner.query(`DELETE FROM "categories" WHERE name IN ('Engineering', 'Design', 'Product', 'Marketing')`);
